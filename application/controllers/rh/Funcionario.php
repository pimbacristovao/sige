...
class Funcionario extends CI_Controller
{
	...
	public function guardar_novo_funcionario()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			redirect('rh/funcionario/criar_utilizador?id_funcionario=' . $id_funcionario);
		}
	}
	/* ================== APAGAR FUNCIONARIOS ================== */
	...
	{		
		...
		$this->load->view('layout/cabecalho_rh');
		$this->load->view('layout/menu_lateral_rh');
		$this->load->view('conteudo/_rh/_utilizador/criar_utilizador', $dados);
    ...
    */
	...
	public function mapa_assiduidade_individual()
	{
		...
		$this->db->from('assiduidade_funcionario');												 				// da tbl assiduidade_funcionario
		$this->db->where("funcionario_id", $funcionario);									 					// onde
		$this->db->where("mes_ano", $mes);									 										// onde
		$this->db->join('funcionario', 'funcionario.id_funcionario = assiduidade_funcionario.funcionario_id');	// join ano funcionario e assiduidade_funcionario
	...
	public function justificar_faltas($id_assiduidade)
	{
		...
		$this->db->join('funcionario', 'funcionario.id_funcionario = assiduidade_funcionario.funcionario_id');			// join tbl funcionario e assiduidade_funcionario
		$dados["funcionario_inf"] = $this->db->get()->row();										// retorna 1 linha
		/* ------------------------------------------------------------------------------------------------------------- */
		$this->db->select('*');																// select tudo
		$this->db->from('assiduidade_alunos');													// da tbl assiduidade_alunos
		$this->db->where("id_assiduidade", $id_assiduidade);									// onde
		$this->db->order_by("data", "asc");  												// Ordenar a travez da data
		// TODO: modificar função justificar_falta (inserir registros na tbl aula)
		$this->db->join('aluno', 'aluno.id_aluno = assiduidade_alunos.aluno_id');		 		// Join tbl aluno e assiduidade_alunos
		$this->db->join('anolectivo', 'anolectivo.id_ano = assiduidade_alunos.anolectivo_id'); 	// Join anolectivo e assiduidade_alunos
		$this->db->join('turma', 'turma.id_turma = assiduidade_alunos.turma_id');		 		// Join turma e assiduidade_alunos
	...
	public function mapa_assiduidade_funcionario()
	{
		...
		/*-----------------------------------------------------------------------------------------------------------------------------*/
		$this->db->select_sum('falta');							// seleciona a soma dos valores da coluna falta e retorna em uma linha
		$this->db->select_sum('justificacao');					// seleciona a soma dos valores da coluna justificacao e retorna em uma linha
		$this->db->from('assiduidade_funcionario');				// da tabela assiduidade_funcionario
		$this->db->where('mes_ano', $mes);									// filtro - mes_ano
		$this->db->group_by('assiduidade_funcionario.funcionario_id');
		$this->db->order_by("nivel_acesso", "asc");
		$this->db->join('funcionario', 'funcionario.id_funcionario = assiduidade_funcionario.funcionario_id'); // join tbl funcionario e assiduidade_funcionario	
	...
	public function Recortar()
	{
		...
		$configUpload['upload_path']   = FCPATH . "assets/upload";
		...
		if (!$this->upload->do_upload('imagem')) {
			...
			$configCrop['new_image'] = FCPATH . "assets/upload";
			// Proporção
	...
	public function ajaxwebcam()
	{
		...
		$fp = fopen('assets/upload/' . $fotografia, 'w');
		fwrite($fp, $unencoded);
		fclose($fp);
		$funcionario = array(
			"id_funcionario" => $id,
